  ...
  function handleWeatherCheck(e) {
    ...
    if (cityWeathers.length > 0) {
      ...
      if (filteredArray.length > 0) {
        setMessage(
          `You already know the weather for ${inputValue} ...otherwise be more specific by providing the country code as well 😉`
        );
        setInput("");
        setIsLoading(false);
        return;
              ...
              cityWeathers.map((cityWeather) => {
                return (
                  <CityWeather
                    ...
                    key={`${cityWeather.name} ${cityWeather.sys.country}`}
                  />
                );
              })}
